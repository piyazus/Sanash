...
Usage:
  python scripts/run_door_apc.py --plan          # selection size only
  python scripts/run_door_apc.py --fetch         # download (respects cap)
  python scripts/run_door_apc.py --evaluate      # process + evaluate + write
"""
...
INDEX = os.path.join(
    os.path.dirname(__file__), "..", "data", "index", "beintelli_v1_zip_index.json"
)
...
def main() -> int:
    ...
    os.makedirs(
        os.path.join(os.path.dirname(__file__), "..", "paper", "results"), exist_ok=True
    )
    out_path = os.path.join(
        os.path.dirname(__file__), "..", "paper", "results", "apc_validation.json"
    )
