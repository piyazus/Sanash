...
Provenance rule as in `docs/findings.md`: every number is either measured
(cross-referenced to findings.md / experiments/log.md), or marked ASSUMED
(reasonable default, replace with real data) or TBD (must be sourced, see
`docs/trade_study.md`). The assumptions register at the end of this file
...
- **Phase 2 (dashed in all figures, dormant):** one wide-angle salon RGB
  ...
  permission; the model assets carry the same DORMANT banner in `src/sanas/`
  (log 2026-08-10).
...
TBD (see docs/trade_study.md unless noted):
