...
> Примечание при синхронизации в vault (23.08.2026): раздел про железо ниже
> (Hikvision RGB-камера, 4 камеры) устарел. Актуальное решение —
> `sanas_cv_track_conclusions.md`, раздел 8: RealSense depth-камеры, 2 шт.
> Рейтинг профессоров в разделе 8 этого документа тоже устарел — актуальный
> список см. `SANASH_consolidated_researcher_list.md`.
